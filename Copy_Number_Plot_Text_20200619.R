...
# SacCer3 chromosome lengths cumulative
...
# Chromosome roman numeral identifier
ChromRomanList <- c("I",
                    "II",
                    "III",
                    "IV",
...
## Load CN data from shell script
...
## Subsets to chromosome SubSetOn
...
## List of all genes from YeastMine
...
## Generates ggplot variable for the time course data
...
ReplicatePlot <- ggplot() +
  ...
  geom_point(data=subE01, aes(x=V2 + (Shift*2) + 100000, y=V3, color="A", fill="A", alpha=0.5))+
  geom_point(data=subE03, aes(x=V2 + (Shift*3) + 100000, y=V3, color="D", fill="D", alpha=0.5))+
  geom_point(data=subWLP001, aes(x=V2 + (Shift*4) + 200000, y=V3, color="A", fill="A", alpha=0.5))+
  geom_point(data=subDrake, aes(x=V2 + (Shift*5) + 200000, y=V3, color="D", fill="D", alpha=0.5))+
  geom_point(data=subRedCircle, aes(x=V2 + (Shift*6) + 200000, y=V3, color="D", fill="D", alpha=0.5))+
  scale_color_manual(
    name = NULL,
    values=darken(c("A"='#33b4ff',
                    "D"='#ffbe33')), 0.3) +
  scale_fill_manual(
    ...
    values = c("A"='#33b4ff',
               "D"='#ffbe33')) +
  geom_vline(xintercept = 2916956 + (Shift*1) - 42000, linetype = "solid", color = "grey80") +
  geom_vline(xintercept = 2916956 + (Shift*2) - 42000 + 50000, linetype = "solid", color = "grey80", size = 1) +
  geom_vline(xintercept = 2916956 + (Shift*3) - 42000 + 100000, linetype = "solid", color = "grey80") +
  geom_vline(xintercept = 2916956 + (Shift*4) - 42000 + 150000, linetype = "solid", color = "grey80", size = 1) +
  geom_vline(xintercept = 2916956 + (Shift*5) - 42000 + 200000, linetype = "solid", color = "grey80") +
  geom_vline(xintercept = 2916956 + (Shift*6) - 42000 + 200000, linetype = "solid", color = "grey80") +
...
#  annotate("text", x = 3175000  + (Shift * 1), y = 1.6, family = "sans", label = round(median(subPD140$V3),2), color = "black") +
  annotate("point", x = 3175000 + (Shift * 2) + 100000, y = median(subE01$V3), color = "black", size = 3) +
#  annotate("text", x = 3175000  + (Shift * 2), y = 1.6, family = "sans", label = round(median(subE01$V3),2), color = "black") +
  annotate("point", x = 3175000 + (Shift * 3) + 100000, y = median(subE03$V3), color = "black", size = 3) +
#  annotate("text", x = 3175000  + (Shift * 3), y = 1.6, family = "sans", label = round(median(subE03$V3),2), color = "black") +
  annotate("point", x = 3175000 + (Shift * 4) + 200000, y = median(subWLP001$V3), color = "black", size = 3) +
#  annotate("text", x = 3175000  + (Shift * 4), y = 1.6, family = "sans", label = round(median(subWLP001$V3),2), color = "black") +
  annotate("point", x = 3175000 + (Shift * 5) + 200000, y = median(subDrake$V3), color = "black", size = 3) +
#  annotate("text", x = 3175000  + (Shift * 5), y = 1.6, family = "sans", label = round(median(subDrake$V3),2), color = "black") +
  annotate("point", x = 3175000 + (Shift * 6) + 200000, y = median(subRedCircle$V3), color = "black", size = 3) +
  annotate(geom = 'segment',
           y = Inf, yend = Inf,
           x = -Inf, xend = Inf,
           color = "grey80",
  ...
  scale_x_continuous(name = "Genome Position",
                     ...
                     breaks = c(3177956,
                                3177956+Shift,
                                3177956+Shift*2 + 100000,
                                3177956+Shift*3 + 100000,
                                3177956+Shift*4 + 200000,
                                3177956+Shift*5 + 200000,
                                3177956+Shift*6 + 200000),
                     labels = c("Repitch 1\n\n",
                                "Repitch 29\n\n",
                                "Repitch 1\n\n",
                                "Repitch 15\n\n",
...
## Uses ggdraw to combine previous plotting elements
pdf('//Volumes/GoogleDrive/My\ Drive/PostdocBrewing_Manuscript/Figures/CopyNumberPlot_20200622.pdf',height=12, width=12, title="Post-Doc_Brewing")
ggdraw() +
  draw_plot(WholeGenomePlot, x = 0, y = 0.70, height = 0.30, width = 1.0) +
  draw_plot(TimeCoursePlot, x = 0, y = 0.4, height = 0.28, width = 0.42) +
  draw_plot(ReplicatePlot, x = 0.42, y = 0.4, height = 0.28, width = 0.58) +
  draw_plot(AFPlot, x = 0.0, y = 0, height = 0.36, width = 0.75) +
