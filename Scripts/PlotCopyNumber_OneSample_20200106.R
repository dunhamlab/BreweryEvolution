...
pdf(paste(args[1],'/',args[2],'/',args[2],'_CopyNumber_1000bp.pdf',sep=""),height=5, width=18)

hetData <- read.table(paste(args[1],'/',args[2],'/CNV_new_1000bp/',args[2],'_1000bp_norm.wig',sep=""), sep='\t', head=FALSE)
#hetData1 <- read.table(paste(args[1],'/',args[3],'/CNV_new_1000bp/',args[3],'_1000bp_norm.wig',sep=""), sep='\t', head=FALSE)
...
hetPlot <- ggplot() + 
  geom_jitter(data=hetData,
              aes(x=hetData$V2,
                  ...
                  shape='1',
                  size='1',                  
                  color = ifelse(hetData$V1 %% 2 == 0, "A", "B"),
                  fill = ifelse(hetData$V1 %% 2 == 0, "A", "B")),
                  alpha = 0.5) +
  scale_color_manual(
    name = NULL,
